...
if ( ! function_exists( '_st_admin_display_google_section' ) ) {
	...
	<?php } ?>
		...
		<p>
			<input type="checkbox" name="st-data680" value="yes" <?php if ( $GLOBALS["stdata680"] === 'yes' ) {
				echo 'checked';
			} ?>>画像（PR）にする</p>
	...
	<p><b>【記事タイトル】</b></p>
		...
		<?php if ( defined( 'ST_LAZY_LOAD' ) ) { // プラグイン有効時 ?>
			...
			<p style="margin: -1em 0 0 2em;">
				...
				<label><input type="checkbox" name="st-data330" value="yes"<?php checked( $GLOBALS['stdata330'] === 'yes' ); ?>> &lt;script&gt;にも対応させる（※defer属性を追加）</label><br>
				<?php if ( $GLOBALS['stdata330'] === 'yes' ): ?>
					<label><input type="hidden" name="st-data656" value="no"><input type="checkbox" name="st-data656" value="yes"<?php checked( $GLOBALS['stdata656'] === 'yes' ); ?>> &lt;script&gt;を遅延読み込みする（※Google Adsense, X 旧Twitter, Instagram）</label>
				<?php else: ?>
					<label><input type="hidden" name="st-data656" value="<?php echo esc_attr( $GLOBALS['stdata656'] ); ?>"><input type="checkbox" name="st-data656" value="yes"<?php checked( $GLOBALS['stdata656'] === 'yes' ); ?> disabled> &lt;script&gt;を遅延読み込みする（※Google Adsense, X 旧Twitter, Instagram）</label>
				<?php endif; ?>
			</p>
